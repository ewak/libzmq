...
int main (void)
{
    ...
    assert (rc == 0);
    rc = zmq_bind (to, "sctp://127.0.0.1:6575");
    assert (rc == 0);
    ...
    rc = zmq_connect (from, "sctp://localhost:5576");
    assert (rc == 0);
    // This pipe will
    rc = zmq_connect (from, "sctp://localhost:6575");
    assert (rc == 0);
    ...
    assert (to);
    rc = zmq_bind (to, "sctp://127.0.0.1:5580");
    assert (rc == 0);
    ...
    rc = zmq_connect (from, "sctp://localhost:5581");
    assert (rc == 0);
    // Connect to the valid socket
    rc = zmq_connect (from, "sctp://localhost:5580");
    assert (rc == 0);
    ...
    assert (rc == 0);
    rc = zmq_bind (backend, "sctp://127.0.0.1:5580");
    assert (rc == 0);
    rc = zmq_connect (frontend, "sctp://localhost:5580");
    assert (rc == 0);
