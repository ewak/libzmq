...
int zmq::ip_address_t::resolve_nic_name (const char *nic_, bool ipv6_)
{
...
int zmq::ip_address_t::resolve_interface (const char *interface_, bool ipv6_)
{
...
int zmq::ip_address_t::resolve_hostname (const char *hostname_, bool ipv6_)
{
    //  Set up the query.
...
zmq::ip_address_t::ip_address_t ()
{
    memset (&address, 0, sizeof (address));
}

zmq::ip_address_t::ip_address_t (const sockaddr *sa, socklen_t sa_len)
{
...
int zmq::ip_address_t::resolve (const char *name_, bool local_, bool ipv6_)
{
...
int zmq::ip_address_t::to_string (std::string &addr_, const std::string &protocol)
{
    ...
    if (address.generic.sa_family == AF_INET6) {
        std::stringstream s;
        s << protocol << "://[" << hbuf << "]:" << ntohs (address.ipv6.sin6_port);
        addr_ = s.str ();
    }
    else {
        std::stringstream s;
        s << protocol << "://" << hbuf << ":" << ntohs (address.ipv4.sin_port);
        addr_ = s.str ();
    };
    return 0;
}

const sockaddr *zmq::ip_address_t::addr () const
{
    return &address.generic;
}

socklen_t zmq::ip_address_t::addrlen () const
{
    if (address.generic.sa_family == AF_INET6)
...
unsigned short zmq::ip_address_t::family () const
#else
sa_family_t zmq::ip_address_t::family () const
#endif
{
    return address.generic.sa_family;
}

zmq::ip_address_mask_t::ip_address_mask_t () :
    ip_address_t ()
{
    address_mask = -1;
}

int zmq::ip_address_mask_t::mask () const
{
    return address_mask;
}

int zmq::ip_address_mask_t::resolve (const char *name_, bool ipv6_)
{
    ...
    int rc = ip_address_t::resolve_hostname (addr_str.c_str (), ipv6_);
    if (rc != 0)
        return rc;
...
int zmq::ip_address_mask_t::to_string (std::string &addr_, const std::string &protocol)
{
...
bool zmq::ip_address_mask_t::match_address (const struct sockaddr *ss, const socklen_t ss_len) const
{
