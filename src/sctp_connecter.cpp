...
void zmq::sctp_connecter_t::process_plug ()
{
    if (delayed_start)
        add_reconnect_timer ();
    else
...
void zmq::sctp_connecter_t::add_reconnect_timer()
{
...
int zmq::sctp_connecter_t::get_new_reconnect_ivl ()
{
...
int zmq::sctp_connecter_t::open ()
{
    ...
    //  Create the socket.
    s = open_socket (addr->resolved.ip_addr->family (), SOCK_STREAM, IPPROTO_SCTP);
...
void zmq::sctp_connecter_t::close ()
{
    zmq_assert (s != retired_fd);
#ifdef ZMQ_HAVE_WINDOWS
